...
uint8_t g_backlight_mode          = BACKLIGHT_MODE_TRADITIONAL;
...
void device_params_load_from_nv(void) {
    ...
    st = hal_nvm_read(NV_ITEM_BACKLIGHT_MODE, sizeof(value), (uint8_t *)&value);
    if (st == HAL_NVM_SUCCESS) {
        g_backlight_mode = value;
    }
}
...
void device_params_set_backlight_mode(uint8_t value) {
    g_backlight_mode = value;
    hal_nvm_write(NV_ITEM_BACKLIGHT_MODE, sizeof(value), (uint8_t *)&value);
}
